...
def alphabeta(state, alpha, beta):
    #this was just the alphabeta code for tic tac toe 
    
    if (len(state.moves)==state.size*state.size or state.detect_five_in_a_row()!=EMPTY) or state.get_captures(BLACK)>=10 or state.get_captures(WHITE)>=10:
        state.win_person = state.detect_five_in_a_row()
        
        #state.win_move=state.moves[-1]
        #print(state.detect_five_in_a_row())
