...
class GoBoard(object):
    ...
    def play_move(self, point: GO_POINT, color: GO_COLOR) -> bool:
        ...
        self.moves.append(point) #list of the points for undo
        ...
        self.captured_move.append(temp)
        return True
    ...
    def undo(self)->None:
        ...
        #checks if it is an empty board
        if len(self.get_empty_points())==self.size*self.size:

            return
        #gets the opp color to change the color after you do it
        ...
        places_where_captures=self.captured_move.pop() #all the points that were captured per turn, also a class variable
        if len(places_where_captures)!=0: #nothing was captured
           
            if self.current_player==WHITE:
                self.black_captures=self.black_captures-len(places_where_captures) #reason why its black is because it automatically goes to th enext turn
            else:
        ...
        self.current_player=opp_color #dont understand the logistics, may get rid of this
        #error handling
        if self.moves!=[]: 
            #self.last_move = self.last2_move
        
            self.last_move=self.moves[len(self.moves)-1]
            self.last2_move=self.moves[len(self.moves)-2]
          
        else:
            self.last_move=NO_POINT
